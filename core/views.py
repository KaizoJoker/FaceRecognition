...
from .models import Lecturer, LastFace
...
def login_user(request):
    ...
    if request.method == "POST":
         ...
         if user is not None:
            ...
            messages.success (request,("The password or username is uncorrect.."))
...
def lecturer(request):
    form = Lecturer
    if request.method == 'POST':
        form = Lecturer(request.POST, request.FILES)
        if form.is_valid():
            form.save()
...
def index(request):
    ...
    all_members = Lecturer.objects.all
